...
Import statements for all the packages and components we'll be using in this lab:
...
16. We could also map this dataset onto a Pandas `DataFrame` to see it in a tabular structure/format.

17. Code to create and display a Pandas `DataFrame` for the iris data:
...
# create dataframe from data in X_train, labelling columns using the strings in iris_dataset.feature_names
